...
import { useLiveTable, bulkUpsert, updateByKey } from '../lib/tables';
import type { ShiftClassType, IEmployee, IShiftRosterEntry } from '../types';
import { useAuth } from '../context/AuthContext';
import { useToast } from '../context/ToastContext';
import { logUserAction } from '../services/audit-log-service';

/**
 * Sắp Xếp Ca realtime qua Supabase (thay JSON file dept + HR_Data):
 * - vinh/nguyetanh/han (Dept Admin): sắp ca phòng mình -> ghi TRỰC TIẾP vào
 *   shift_rosters (RLS giới hạn đúng departmentScope).
 * - kieu/hoa (Master): thấy ngay qua realtime, đối soát ở tab Tiếp Nhận.
 */
...
export const ShiftAssignmentPage: React.FC = () => {
  ...
  // Chế độ xem: kieu/hoa mặc định ở 'received' (đối soát realtime toàn công ty)
  // vinh/nguyetanh/han mặc định ở 'manual' (sắp ca phòng mình, ghi trực tiếp Supabase)
  ...
  // Live queries từ Supabase (realtime)
  const employees = useLiveTable<IEmployee>('employees');
  const shiftRosters = useLiveTable<IShiftRosterEntry>('shiftRosters');
  // Nhật ký nộp ca kiểu file đã bỏ — Supabase realtime thay thế hoàn toàn.
  const shiftSubmissions: any[] = [];
  ...
  const handleBulkApply = () => {
    ...
    info('Đã áp dụng ca hàng loạt', `Đã gán ${bulkShift} cho ${selectedEmployeeIds.size} nhân viên đã chọn. Bấm "Lưu Ca Làm Việc" để ghi lên Supabase.`);
  };

  // Lưu ca trực tiếp lên Supabase (RLS giới hạn đúng phòng ban)
  const handleSaveAndSync = async () => {
    ...
    try {
      ...
      // Ghi TRỰC TIẾP lên Supabase — RLS giới hạn dept admin đúng phòng mình,
      // kieu/hoa thấy ngay qua realtime, không còn file dept / HR_Data.
      await bulkUpsert('shiftRosters', toSave as any);
      for (const [empId, shiftCode] of entries) {
        try {
          await updateByKey('employees', empId, { shiftClassId: shiftCode } as any);
        } catch { /* ignore */ }
      }

      success(
        'Đã lưu ca làm việc',
        `Đã lưu ${toSave.length} ca (${entries.length} NV × ${dateRange.length} ngày) lên Supabase. Mọi máy thấy ngay theo thời gian thực.`
      );
      ...
      if (session) {
        logUserAction({
          ...
          details: `Sắp ca ${dateRange[0]} -> ${dateRange[dateRange.length - 1]} (Supabase realtime, ${entries.length} NV)`
        }).catch(console.error);
      }
    } catch (err: any) {
    ...
    <div className="p-6 w-full space-y-6 flex-1 flex flex-col font-sans">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {isHostOrHR && viewMode === 'received'
                  ? 'Đối soát ca 3 phòng theo thời gian thực từ Supabase (không còn file dept / HR_Data).'
                  : 'Sắp ca cho nhân viên phòng bạn. Bấm Lưu để ghi trực tiếp lên Supabase, kieu/hoa thấy ngay.'}
              </p>
            </div>
          </div>
        </div>
      ...
      {isHostOrHR && viewMode === 'received' ? (
        <div className="space-y-6 flex-1 flex flex-col">
          {/* Realtime Supabase — ca dept ghi trực tiếp, không còn hộp thư file */}
          <div className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm space-y-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                <Inbox className="w-4 h-4 text-emerald-600" />
                <span>Ca các phòng (realtime Supabase)</span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-100 text-emerald-700 border border-emerald-200">live</span>
              </h3>
            </div>
            <p className="text-[11px] text-slate-500">
              vinh / han / nguyetanh sắp ca phòng mình ở tab “Sắp Ca Thủ Công” — dữ liệu ghi thẳng
              lên Supabase (RLS giới hạn đúng phòng) và hiện ngay tại bảng đối soát bên dưới.
              Không còn nộp file dept / quét HR_Data.
            </p>
          </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
            <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  ...
                  <span>Lưu Ca Làm Việc ({mode === 'day' ? '1 ngày' : mode === 'week' ? '7 ngày' : 'cả tháng'})</span>
                </button>
              </div>
            </div>
            ...
            <div className="text-[11px] text-slate-500 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 flex items-center justify-between">
              <span>
                ...
                Bấm Lưu để ghi trực tiếp lên Supabase (kieu/hoa thấy ngay theo thời gian thực).
              </span>

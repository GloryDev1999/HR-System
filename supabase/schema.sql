...
-- Nguyên tắc: Supabase Auth là source-of-truth DUY NHẤT (credential + RBAC).
--             Role/scope nằm trong auth.users.raw_app_meta_data (server-side),
--             RLS đọc trực tiếp JWT — KHÔNG bảng profiles trung gian.
...
-- ----------------------------------------------------------------------------
-- §0. QUYẾT ĐỊNH AUTH: CHUẨN SUPABASE, KHÔNG TABLE TRUNG GIAN.
--   - Credential + RBAC đều nằm trong auth.users:
--     raw_app_meta_data = {username, display_name, role, department_scope}
--     (chỉ service_role/Dashboard sửa được → an toàn cho RLS).
--   - RLS đọc JWT: (auth.jwt() -> 'app_metadata' ->> 'role').
--     KHÔNG dùng user_metadata cho phân quyền (user tự sửa được),
--     KHÔNG dùng auth.role() đã deprecated.
--   - Tạo user: Dashboard Authentication → Add user → sửa App Metadata,
--     hoặc Admin API với app_metadata (xem DEPLOY.md). Frontend signIn
--     bằng email + password như web app chuẩn.
-- ----------------------------------------------------------------------------
...
-- RLS đọc role/scope từ JWT app_metadata (không bảng profiles).
-- Quy ước: role IN ('AD System','HR Manager','HR Admin') = full HR;
-- dept admin (scope WH/Production/QC) chỉ thấy phòng mình.
